...
### Make table of total transfer numbers and rates (needed for earth.num)
...
### Make tables of collision rates (needed for 4GyrTables)
#source('MoonRateTable.R')

### Make tables of 4 Gyr totals
#if (vers!=3) source('MakeLittleTable.R')
...
###### IF using .aei files #######
ObjInd=0:1499
DoImages			= 1	#(0:100*100)*1000	# yr at which to make images
DoAEPlot			= 1	#(0:100*100)*1000	# yr at which to make images
# Read aei files
source('ReadAei.R')

# Make pretty pictures of flight
source('3ShotsSeparately.R')


# Top-down image
#if ( length(DoImages)>1 | DoImages[1]>0 ) {
#	source('Images.R')
#	}
